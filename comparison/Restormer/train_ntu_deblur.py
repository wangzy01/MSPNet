...
def main():
    ...
    pretrain_g = opt['path'].get('pretrain_network_g')
    if pretrain_g is not None:
        print(f"[INFO] Detected pretrain_network_g, disabling auto-resume")
        print(f"[INFO] Will only load model weights: {pretrain_g}")
    else:
        ...
        if len(states) > 0:
            ...
            print(f"[INFO] Auto-resuming from: {resume_state}")
