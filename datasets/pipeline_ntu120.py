...
class Flip:
    ...
    def _flip_single_result(self, results):
        """Perform flip on a single result dictionary."""
        _init_lazy_if_proper(results, self.lazy)
        if 'keypoint' in results:
            assert not self.lazy, ('Keypoint Augmentations are not compatible '
                                   'with lazy == True')
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict or list of dict): The resulting dict to be modified and passed
                to the next transform in pipeline.
        """
        if isinstance(results, (tuple, list)):
            return [self._flip_single_result(r) for r in results]
        else:
            return self._flip_single_result(results)
...
class Normalize:
    ...
    def __call__(self, results):
        if isinstance(results, (tuple, list)):
            ...
            return self._process_single_result(results)
...
class Normalize_gray:
    ...
    def __init__(self, mean, std, to_bgr=False, adjust_magnitude=False):
        ...
        if not isinstance(std, Sequence):
            raise TypeError(
                f'Std must be list, tuple或 np.ndarray, but got {type(std)}')
    ...
    def _process_single_result(self, results):
        ...
        if modality == 'RGB':
            ...
            for img in imgs:
                gray_img = img[:, :, 0]  # Assuming all channels are the same, use the first channel
                gray_img = (gray_img - self.mean[0]) / self.std[0]
                img[:, :, 0] = gray_img
                img[:, :, 1] = gray_img
                img[:, :, 2] = gray_img
            results['imgs'] = imgs
...
class ColorJitter:
    ...
    def __call__(self, results):
        if isinstance(results, (tuple, list)):
            # 如果是元组或列表，遍历每个结果分别应用
            return [self._apply_color_jitter(r) for r in results]
        else:
            # 否则直接应用
            return self._apply_color_jitter(results)
    
    def _apply_color_jitter(self, results):
        imgs = results['imgs']
        v = random.random()
        if v < self.p:
...
class Compose3:
    ...
    def __call__(self, data):
        if isinstance(data, (tuple, list)):
            results_list = data
            for t in self.transforms:
                results_list = [t(results) for results in results_list]
                if any(r is None for r in results_list):
                    return None
            return tuple(results_list) if isinstance(data, tuple) else results_list
        else:
            for t in self.transforms:
                data = t(data)
                if data is None:
...
class Getdata:
    ...
    def __call__(self, results):
        """Perform the data initialization.
    
        Args:
            results (dict | list[dict]): The resulting dict(s) to be modified and passed
                to the next transform in pipeline.
        """
        if isinstance(results, (tuple, list)):
            return [self._process_single_result(r) for r in results]
    ...
    def _process_single_result(self, result):
        num_files = len(os.listdir(result['filename']))
        result['total_frames'] = num_files
        return result
...
class Getdata2:
    ...
    def __call__(self, results, results2):
        """Perform the Decord initialization.
        ...
        results2['total_frames'] = num_files
...
class Load_IFNS:
    ...
    def __call__(self, results):
        ...
        video_a = results['filename']
        video = os.path.join("/data2/NTU_data/resize_mb_fd_4", os.path.basename(video_a))
        video_3 = os.path.join("/data2/NTU_data/resize_mb_avg", os.path.basename(video_a))
        ...
        def load_frame(video, idx):
            frame_path = os.path.join(video, str(idx) + '.jpg')
        ...
        if self.train:
            for jj in range(num_clips):
                # 修改，去掉第0帧和最后1帧
                start_idx = max(1, int(ff * jj / num_clips))
                end_idx = min(last_idx, int(ff * (jj + 1) / num_clips))
                if start_idx >= end_idx:
                    iii = start_idx  # or any fallback value, e.g., start_idx or end_idx
                else:
                    iii = int(np.random.randint(start_idx, end_idx))

                # iii = int(np.random.randint(max(1, int(ff * jj / num_clips)), min(last_idx, int(ff * (jj + 1) / num_clips))))
                frame_idx = iii
                # 加载前一帧、当前帧和后一帧

                frame_1 = load_frame(video, max(0, iii - 1))
                frame_2 = load_frame(video, iii)
                frame_3 = load_frame(video_3, iii)
                # st()

                imgs.append(frame_1)
                imgs2.append(frame_2)
                imgs3.append(frame_3)

        else:
            for jj in range(num_clips):
                # 修改，去掉第0帧和最后1帧
                iii = int((int(ff * jj / num_clips) + int(ff * (jj + 1) / num_clips)) / 2)
                iii = min(last_idx, max(1, iii))
                frame_idx = iii
                # 加载前一帧、当前帧和后一帧
                frame_1 = load_frame(video, max(0, iii - 1))
                frame_2 = load_frame(video, iii)
                # frame_3 = load_frame(video, min(last_idx, iii + 1))
                frame_3 = load_frame(video_3, iii)
        ...
        if ff < self.num_clips:
            frame_idx = last_idx
            frame_1 = load_frame(video, max(0, frame_idx - 1))
            frame_2 = load_frame(video, frame_idx)
            frame_3 = load_frame(video_3, frame_idx)
            # frame_3 = load_frame(video, min(last_idx, frame_idx + 1))
            imgs.append(frame_1)
            imgs2.append(frame_2)
            imgs3.append(frame_3)
...
class FormatShape:
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict | list[dict] | tuple[dict]): The resulting dict(s) to be modified and passed
                to the next transform in pipeline.
        """
        if isinstance(results, (tuple, list)):
            return [self._format_single_result(r) for r in results]
        else:
            return self._format_single_result(results)

    def _format_single_result(self, results):
        """Formats a single results dict.
...
class Collect:
    ...
    Args:
        ...
        meta_keys (Sequence[str]): Keys that are collected under meta_name.
            ...
            - "flip_direction": a str in ("horizontal", "vertical") to
                indicate if the image is flipped horizontally or vertically.
    ...
    def _collect_single_result(self, results):
        """Collect data from a single results dict."""
        data = {}
    ...
    def __call__(self, results):
        ...
        """
        if isinstance(results, (tuple, list)):
            return [self._collect_single_result(r) for r in results]
        else:
            return self._collect_single_result(results)
...
class ToTensor:
    ...
    def _to_tensor_single_result(self, results):
        """Convert some values in a single results dict to `torch.Tensor` type."""
        for key in self.keys:
    ...
    def __call__(self, results):
        ...
        """
        if isinstance(results, (tuple, list)):
            return [self._to_tensor_single_result(r) for r in results]
        else:
            return self._to_tensor_single_result(results)
