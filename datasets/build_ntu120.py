            ...
            if self.BenchMark == "NTU120-XSub":
                if self.train:
                    if subject in NTU120_CSub_Train: 
                        ...
                        self.video_infos.append(dict(filename=filename, label=label, name=name, subject=id))
                else:
...
def build_dataloader(logger, config):
    train_pipeline_ntu = [
        ...
        # dict(type='Normalize_gray', **img_norm_cfg),  ### 灰度图也可以不使用这个版本
        dict(type='Normalize', **img_norm_cfg), 
        dict(type='FormatShape', input_format='NCHW'),
        ###########
        dict(type='Collect', keys=['imgs', 'label', 'subject'], meta_keys=[]),
        dict(type='ToTensor', keys=['imgs', 'label', 'subject']),
        ###########
    ]
